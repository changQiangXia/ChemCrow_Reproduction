...
class RXN4Chem(BaseTool):
    ...
    def __init__(self, rxn4chem_api_key, project_id: Optional[str] = None):
        """Init object."""
        super().__init__()
        ...
        self.rxn4chem.project_id = (
            project_id
            or os.getenv("RXN4CHEM_PROJECT_ID")
            or "655b7b760fb57c001f25dc91"
        )
...
class RXNRetrosynthesis(RXN4Chem):
    ...
    def __init__(
        ...
        """Init object."""
        super().__init__(rxn4chem_api_key, project_id=project_id)
        self.openai_api_key = openai_api_key
        self.summary_model = summary_model or os.getenv(
            "CHEMCROW_SUMMARY_MODEL", "gpt-3.5-turbo-16k"
        )
    ...
    def get_action_sequence(self, path):
        ...
        # RXN materializes node actions gradually and rate-limits aggressively.
        # Poll a few times with low request density and keep the first actions found.
        for wait_factor in (2, 4, 8):
            sleep(self.sleep_time * wait_factor)
            for node in nodeids:
                if node in real_nodes:
                    continue
                try:
                    node_resp = self._normalize_reaction_settings(
                        self.rxn4chem.get_reaction_settings(
    ...
    def get_reaction_settings(self, synthesis_id: str, node_id: str):
        ...
        return self._normalize_reaction_settings(response)

    def _normalize_reaction_settings(self, response):
        if isinstance(response, dict) and "actions" in response and "product" in response:
            return response
        if isinstance(response, dict) and "response" in response:
            ...
            if isinstance(nested, dict):
                ...
                if isinstance(payload, dict):
                    if "actions" in payload and "product" in payload:
                        return {
